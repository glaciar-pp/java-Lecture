...
# Full Outer Join, Union 사용하여 Left와 Right를 붙여 한번에 보여준다.
SELECT r.`name`, r.debut, l.title, l.lyrics
FROM song AS l
LEFT OUTER JOIN girlgroup AS r
ON l.sid = r.hitSongId
...
SELECT r.`name`, r.debut, l.title, l.lyrics
FROM song AS l
RIGHT JOIN girlgroup AS r
ON l.sid = r.hitSongId;

/* 연습문제*/
# 데뷔일자가 빠른 5개 걸그룹 힛트송은? (그룹명, 곡명, 데뷔일자)
...
# 2007년도에 데뷔한 걸그룹은?(바람직한 방법은 아니지만 위 방법과 동일한 결과를 보여줌)
SELECT `name`,debut FROM girlgroup
